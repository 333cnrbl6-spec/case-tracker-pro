...
export default function IncidentReporter() {
  ...
  const getIncidentStats = (incidentId) => {
    ...
    const relatedEvidence = evidence.filter(e =>
      e.related_incidents?.includes(incidentId)
    );
            ...
            {incidents.map(incident => {
               ...
               return (
                 <Card
                   ...
                   style={{ borderLeftColor: incident.severity === 'critical' ? '#dc2626' : '#f59e0b' }}
                 >
                   <CardContent className="p-6">
                     <div className="flex items-start justify-between mb-4">
                       <div className="flex-1">
                         <h3 className="font-semibold text-lg text-slate-900 mb-2">{incident.title}</h3>
                         <p className="text-sm text-slate-600">{incident.description?.substring(0, 100)}...</p>
                       </div>
                     ...
                     {/* RICS Violations */}
                     {incident.rics_violations && incident.rics_violations.length > 0 && (
                       <div className="mb-4">
                         <p className="text-xs font-semibold text-slate-700 mb-2">RICS Violations:</p>
                         <div className="flex flex-wrap gap-2">
                           {incident.rics_violations.slice(0, 3).map((violation, idx) => (
                             <Badge key={idx} variant="outline" className="text-xs border-red-300 text-red-700 bg-red-50">
                               {typeof violation === 'string' ? violation : violation?.rule_number || 'Unknown'}
                             </Badge>
                           ))}
                           {incident.rics_violations.length > 3 && (
                             <Badge variant="outline" className="text-xs">+{incident.rics_violations.length - 3} more</Badge>
                           )}
                         </div>
                       </div>
                     )}
                     ...
                     <div className="grid grid-cols-2 gap-4 text-xs text-slate-600 mb-4">
                       <div>
                         <span className="font-semibold">Incident Date:</span> {new Date(incident.date).toLocaleDateString('en-GB')}
                       </div>
                       <div>
